...
// Função para explodir o asteroide
  ...
  planets.forEach(planet => {
    ...
    planet.addEventListener('drop', (event) => {
      ...
      if (collidedPlanet && collidedPlanet.classList.contains('planet') && collidedPlanet !== draggedPlanet) {
        simulateCollision(draggedPlanet, collidedPlanet); // Chama a função de colisão
      }
    });
  });
}
...
/*Alternar entre Sistema Solar e Modelo Atômico*/
document.getElementById('toggle-model').addEventListener('click', function () {
  ...
  if (atomModel.style.display === 'none') {
      atomModel.style.display = 'flex';
      body.style.background = '#000';
      this.textContent = 'Alternar para Sistema Solar';
  } else {
      atomModel.style.display = 'none';
      body.style.background = 'black';
      this.textContent = 'Alternar para Modelo Atômico';
  }
});